...
const mailgunFrom = "Campus Ambassador Hub <no-reply@campus.studentembassador.com>";
...
const emailShell = ({ eyebrow, title, intro, actionLabel, actionHref, detail, unsubscribeHref }: { eyebrow: string; title: string; intro: string; actionLabel: string; actionHref: string; detail: string; unsubscribeHref?: string }) => {
  ...
  return `<!doctype html><html lang="pt-BR"><body style="margin:0;background:#f5f7fb;font-family:Arial,'Plus Jakarta Sans',sans-serif;color:#17233a"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="padding:32px 12px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#fff;border:1px solid #d9e2f0;border-radius:24px;overflow:hidden"><tr><td style="height:8px;background:linear-gradient(90deg,#4f46e5 0%,#2563eb 50%,#06b6d4 100%)"></td></tr><tr><td style="padding:34px 38px 22px"><p style="margin:0;color:#4f46e5;font-size:12px;font-weight:700;letter-spacing:1.4px;text-transform:uppercase">${escapeHtml(eyebrow)}</p><h1 style="margin:14px 0 16px;font-size:30px;line-height:1.18;color:#17233a">${safeTitle}</h1><p style="margin:0;color:#41546f;font-size:16px;line-height:1.65">${escapeHtml(intro)}</p><p style="margin:26px 0"><a href="${actionHref}" style="display:inline-block;background:#4f46e5;border:2px solid #17233a;border-radius:12px;box-shadow:4px 4px 0 #17233a;color:#fff;font-weight:700;padding:13px 20px;text-decoration:none">${escapeHtml(actionLabel)} →</a></p><p style="margin:0;color:#60728d;font-size:13px;line-height:1.6">${escapeHtml(detail)}</p></td></tr><tr><td style="padding:18px 38px;background:#f8faff;border-top:1px solid #e6edf7;color:#6b7c94;font-size:12px;line-height:1.5">Campus Ambassador Hub<br/>Projeto independente criado por participantes da comunidade para facilitar conexões entre estudantes. Não é um produto oficial do Google ou da Amplifica.${unsubscribeHref ? ` <a href="${unsubscribeHref}" style="color:#4f46e5">Gerenciar e-mails</a>` : ""}</td></tr></table></td></tr></table></body></html>`;
};
...
export const sendPasswordResetEmail = async (user: User, token: string) => {
  ...
  const name = displayName(user);
  await sendMail({ to: user.email, subject: "Redefina sua senha do Campus Ambassador Hub", html: emailShell({ eyebrow: "Segurança da conta", title: "Vamos recuperar seu acesso.", intro: `Olá, ${name}. Recebemos um pedido para redefinir a senha da sua conta.`, actionLabel: "Criar nova senha", actionHref: url, detail: "O link expira em uma hora e só pode ser usado uma vez. Se não foi você, ignore este e-mail." }), text: `Olá, ${name}. Redefina sua senha: ${url}` });
};
