...
const EventListItem = ({ event, selected }: { event: EventDirectoryItem; selected: boolean }) => <a href={`/events/${event.id}`} className={`block rounded-3xl border-3 bg-white p-5 shadow-hard-black transition-transform hover:-translate-y-0.5 ${selected ? 'border-[#EA4335]' : 'border-[#1e293b]'}`}><div className="flex items-start justify-between gap-3"><span className="inline-flex items-center gap-2 rounded-full bg-[#EBF3FE] px-2.5 py-1 text-[11px] font-black text-[#4285F4]"><CalendarDays size={14} />{dateFormatter.format(new Date(event.startsAt))}</span>{event.capacity ? <span className="text-xs font-bold text-slate-500">{event.participantCount}/{event.capacity}</span> : <span className="text-xs font-bold text-slate-500">{event.participantCount} inscritos</span>}</div><h3 className="mt-3 text-lg font-black leading-snug">{event.title}</h3><EventTags tags={event.tags} className="mt-3" /><p className="mt-3 flex items-start gap-2 text-sm font-medium text-slate-600"><MapPin className="mt-0.5 shrink-0 text-[#EA4335]" size={16} /><span>{event.location}{event.city ? ` · ${event.city}${event.state ? `, ${event.state}` : ''}` : ''}</span></p>{event.organizer && <p className="mt-3 flex items-center gap-2 text-xs font-bold text-slate-500"><GraduationCap size={15} className="text-[#4285F4]" />{event.organizer.universityName}</p>}<span className="mt-4 inline-flex items-center gap-2 text-sm font-black text-[#4285F4]">Ver detalhes <ArrowRight size={16} /></span></a>;
